...
export async function createPerson(formData: FormData) {
  ...
  const person = await db.person.create({
    data: {
      ...
      gender: parsed.gender && String(parsed.gender) !== "" ? (parsed.gender as "MALE" | "FEMALE" | "NOT_SPECIFIED") : null,
      dateOfBirth: parsed.dateOfBirth ? new Date(parsed.dateOfBirth) : null,
      householdId: parsed.householdId || null,
      householdRole: parsed.householdRole && String(parsed.householdRole) !== "" ? (parsed.householdRole as "HEAD" | "SPOUSE" | "CHILD" | "MEMBER" | "OTHER") : undefined,
